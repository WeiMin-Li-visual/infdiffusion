...
def  adjacency_matrix():
    ...
    for i in range(len(edges)):
        A[int(edges[i][1])][int(edges[i][0])] = A[int(edges[i][0])][int(edges[i][1])] = round(
            random.gauss(mean, variance), 2)  # 数据中的节点是从1开始的，而矩阵是从0开始的
        edgeNum[int(edges[i][0])][int(edges[i][1])] =edgeNum[int(edges[i][1])][int(edges[i][0])]= i
    #for j in range(len(edges1)):
        #edgeNum[int(edges1[j][0])][int(edges1[j][1])] = j
    return A,edgeNum


A,edgeNum=adjacency_matrix()
DegreeMatrix=[]#度矩阵
for i in A:
...
def SI_diffusion(percentage,choose_observer_method):
    # timeDelay = []  # 网络中边的时延集合,为每个边生成时延值
    # for edge in range(edges_num):
    #     timeDelay.append(random.gauss(mean, variance))
    ...
    print("源节点所在的分区",SourceNodeInCom)
        ...
        for i in range(len(infected_arr)):
            for j in range(node_num):
                if count == ObserverNum:  # 如果所有的观测节点都感染了，此时停止信息扩散，因为我们只需要知道观测节点的信息就可以了
                    break
                if (A[infected_arr[i]][j] > 0 and j not in infected_arr):  # 如果infected_arr[i]和j两个节点有边连接，并且infected_arr[i]的邻居j不在已感染列表中,邻接矩阵中的值表示时延
                    infectTime[j] = infectTime[infected_arr[i]] + A[infected_arr[i]][j]  # 节点j的感染时间等于节点infected_arr[i]的感染时间加上路径ij的时延值
                    infected_arr.append(j)  # 将这个节点加入到感染节点集合中，下一个时刻可以传染为未感染的邻居节点
                    active_records[len(active_records) - 1].append([infected_arr[i], j, round(infectTime[j], 2)])  # 将感染的节点信息加入该次迭代的列表中
    ...
    sortedComInfectedTime=sorted(communityAvgInfectedTime.items(), key=lambda item: item[1])#对每个社区的平均感染时间进行升序排序[(0, 3.04), (5, 3.53)...]
    print("sortedComInfectedTime",sortedComInfectedTime)
    ...
    #sourceNode源节点
    #CommunitiesList前四个分区列表[分区1，分区2，分区3，分区4]
    #SourceNodeInCom源节点所在的分区
    #AllCandidateObserveNodes所有的候选观测节点[[观测节点，感染时间],[]...]
    #ALLCandidatSourceNode:前四个社区的所有节点
    #AllCandidateObserveNode=sorted(AllCandidateObserveNode, key=lambda x: x[1])[:(int)(len(AllCandidateObserveNode)/2)]
...
def cal_shortest_path_length_list(source,Observes):
...
def ShortestPathLength(Source, ReferenceNode, d):
    ObserveList=[]
    ObserveList.append(ReferenceNode)
    for i in d:
        ObserveList.append(i[0])
    ObserversPathLength=cal_shortest_path_length_list(Source,ObserveList)
    if type(ObserversPathLength)==list:
        ObserveNodeShortestPathLengthList = []  # 除了参考节点以外的节点到源节点的最短路径列表
        for i in range(1,len(ObserveList)):
            ObserveNodeShortestPathLengthList.append(round(mean * (ObserversPathLength[i] - ObserversPathLength[0]), 2))
...
# 计算两个路径相交的边的个数
def calIntersectEdge(edgeList1, edgeList2):
    edges1 = []
    edges2 = []
    ...
    for i in range(observe_num):
        for j in range(observe_num):
            if i >= j:  # 矩阵是对角矩阵，只需要计算一半就可以了
                ...
                varianceMatrix[i][j] = varianceMatrix[j][i] = calIntersectEdge(shortestPathList1,shortestPathList2)  # 两个节点相交的边
...
def GM(Community,ObserverInfectedList):
    Observes=[a[0] for a in ObserverInfectedList]
    ...
    for observeIndex in range(1, len(ObserverInfectedList)):
        d.append([])
        timeDelay = round((ObserverInfectedList[observeIndex][1] - ReferenceNodeInfor[1]), 2)
    ...
    for AssumedSource in Community:#候选源节点集合中每个节点为源时计算最大似然函数
        u=ShortestPathLength(AssumedSource,ReferenceNodeInfor[0],d)#假设的源节点，参考的源节点，真实的时延向量（目前是为了计算观测节点） 得到理论的时间延迟
        if type(u)!=list:
            continue
